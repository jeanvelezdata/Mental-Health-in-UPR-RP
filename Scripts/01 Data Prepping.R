...
projectFolder <- "Mental-Health-in-UPR-RP"
outputsFolder <- file.path(projectFolder, "Outputs")
dataFolder <- file.path(projectFolder, "Raw Data")
...
colnames(rp_data) <- c("ID", "START", "END", "EMAIL", "NAME", paste0("CON_COMP_", 1:10), "CONSENT", "SEX", "GENDER", "SEX_ORIENT", 
                       ...
                       "LOST_JOB_CV19", "GOV_ASSIST", "ACA_YR", "CRD_HRS", "FACUL", "ACA_LVL", "GPA_SR", "UNIV_ECON_AST", "TECH_ACSS",
...
# Eliminate variables not necessary or containing information not needed for analysis
...
# SM_TIME_W_BP_YAM
rp_data$SM_TIME_W_BP_YAM <- factor(rp_data$SM_TIME_W_BP_YAM,
                           levels = c("Menos de 2 horas",
                                      "2-3 horas",
                                      "4-6 horas",
                                      "7-12 horas",
                           ...
                           labels = c("<2","2-3","4-6","7-12",
                                      ">12"), ordered = T)
rp_data$SM_TIME_W_BP_YAM <- ifelse(rp_data$SOC_MED == "No" & rp_data$ID >= 148,
                           "0", rp_data$SM_TIME_W_BP_YAM)
...
# Vectorize the function to work with multiple column names
separate_multi_resps <- function(data, cols){
  
  for(i in 1:length(cols)) {
    data <- separate_multi_resp(data, cols[i])
  }
...
# Handle all multi-response variables in the data
rp_data <- separate_multi_resps(rp_data, cols = c("GOV_ASSIST","TECH_ACSS","EDUC_MOD","PREEX_COND",
                                                  "DEP_TRT","ANX_TRT","HALUC_TYPE"))


# Export data up to this point
today <- sub("-", "_", sub("-", "_", Sys.Date()))
write.csv(rp_data, file.path(outputsFolder, paste0("Clean_prepped_data_", today, ".csv")))
